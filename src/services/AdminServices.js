...
const AdminOTPService = async (req) => {
    try{
        ...
        await AdminModel.updateOne({email:email},{$set:{otp:code}},{upsert:true})
...
const VerifyAdminOTPService = async (req) => {
    try{
        let email = req.params.email;
        let otp = req.params.otp;
        let total = await AdminModel.find({email:email,otp:otp}).count('total');
        if(total===1){
            let user_id = await AdminModel.find({email:email,otp:otp}).select('_id')
